...
public struct Settings: Sendable {
    ...
    /// Шаг анализа в отсчётах: через столько новых отсчётов пересчитываются полосы.
    ///
    /// Отсюда и задержка — `frameSlide / частоту`: 512 при 48 кГц даёт новый кадр
    /// каждые 10.7 мс против 21.3 мс у оригинала с его шагом 1024. Окно анализа
    /// при этом остаётся в 2048 отсчётов, кадры просто перекрываются вчетверо,
    /// а не вдвое.
    ///
    /// На частоту отправки не влияет: её ограничивает отправитель, не чаще
    /// 50 пакетов в секунду.
