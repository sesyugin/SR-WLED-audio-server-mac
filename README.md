...
чтобы кто-то мог подслушать поток. Здесь звук читается штатным механизмом самой
...
заново на Swift. По дороге нашлось несколько мест, где оригинал делал не то, что
написано на этикетке.

### Значения в пакете значат то, что называется
...
### Полосы взяты из самой прошивки

Оригинал строил полосы логарифмической сеткой от 40 до 10000 Гц. Звучит разумно,
но у БПФ шаг по частоте постоянный — 23.4 Гц при частоте 48 кГц и окне в 2048
...
И главное: эффекты GEQ, DJ Light, Blurz и Akemi авторы настраивали именно под эту
таблицу — каждый канал светит своим цветом в своей точке ленты.

### Динамика больше не борется сама с собой
...
| Частота отправки | без предела | **не чаще 50/с** | выше прошивка захлёбывается; предел назван в документации WLED-MM |
| Выход из программы | лента застывает | **гасится** | ни ваниль, ни MM не гасят полосы сами |
...
отбросит пакеты как дубли, а лента замрёт до перезагрузки по питанию.
